            ...
            <div class="col-md-12">
                <div id="checkout-form-wizard">
                    ...
                    <section class="card card-body mb-0">
                        {!! Form::open(['route' => ['reviews.update',$review->id]]) !!}
                        @method('PUT')
                        <div class="mb-4">
                        ...
                        <div class="row mb-2">
                            <div class="col-md-12">
                                ...
                                <div class="form-group pb-3">{{ Form::label('slug', 'Slug', ['class' => 'form-label']) }}
                                    {{ Form::text('slug', old('slug',$review->slug), ['class' => 'form-control']) }}
                                    @error('slug')
                                        <span class="text-danger py-2">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="form-group pb-3">
                                    ...
                                    {{ Form::select('rating', [1 => '1 Star', 2 => '2 Star', 3 => '3 Star', 4 => '4 Star', 5 => '5 Star'], old('rating',$review->rating), ['class' => 'form-control','placeholder' => 'select rating...']) }}
                                    @error('status')
                                        <span class="text-danger py-2">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="form-group pb-3">{{ Form::label('review', 'Review', ['class' => 'form-label']) }}
                                    {{ Form::textarea('review', old('review',$review->review), ['class' => 'form-control', 'rows' => 3]) }}
                                    @error('review')
                                        <span class="text-danger py-2">{{ $message }}</span>
                                    @enderror
                                </div>
                        ...
                        <div class="row g-4 mb-4">
                            <div class="col-md-6">
                                {{ Form::submit('UPDATE REVIEW', ['class' => 'btn btn-sm btn-danger px-4 py-2']) }}
                            </div>
                        </div>
                        {!! Form::close() !!}
                    </section>
